...
export class ThermostatPlatformAccessory {
  ...
  constructor(
    ...
    if(device.operationStatus.fanRequest === true) {
      // Fan Controls
  ...
  parseStatus() {
    ...
    if(this.device.operationStatus.fanRequest === true) {
      if (this.deviceFan) {
  ...
  async refreshStatus() {
    try {
      ...
      this.platform.log.debug(`Fetched update for ${this.device.name} from Honeywell API: ${JSON.stringify(this.device.changeableValues)} and Fan: ${JSON.stringify(devicefan)}`);
      this.device = device;
