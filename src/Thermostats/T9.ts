...
export class T9 {
  ...
  doFanUpdate!: any;
  allowedModes!: configTypes.T9Thermostat['allowedModes'];
  heatonlyMode!: any;
  coolonlyMode!: any;
  noAutoMode!: any;
  ...
  async refreshStatus() {
    try {
      ...
      this.platform.log.debug(`Fetched update for ${device.name} from Honeywell API: ${JSON.stringify(this.device.changeableValues)}`);
