...
export class RoomSensorThermostat {
  ...
  constructor(
    ...
    if (device.changeableValues!.heatCoolMode === 'Heat') {
      this.debugLog(
        `Room Sensor Thermostat: ${accessory.displayName} mode: ${
  ...
  parseStatus() {
    ...
    this.CurrentRelativeHumidity =
      this.sensorAccessory.accessoryValue.indoorHumidity;
    ...
    this.debugLog(
      `Room Sensor Thermostat: ${this.accessory.displayName} CurrentHeatingCoolingState: ${this.CurrentHeatingCoolingState}`,
    );
          ...
          if (this.device.deviceModel.startsWith('T9')) {
            if (this.device.groups) {
              ...
              for (const group of groups) {
                const roomsensors = await this.platform.getCurrentSensorData(
                  this.device,
                  group,
                  this.locationId,
                );
                if (roomsensors.rooms) {
                  ...
                  this.debugLog(
                    `Room Sensor Thermostat: ${
                      this.accessory.displayName
                    } roomsensors: ${JSON.stringify(roomsensors)}`,
                  );
                        ...
                        if (accessory.accessoryAttribute) {
                          if (accessory.accessoryAttribute.type) {
                            if (
                              ...
                              this.debugLog(
                                `Room Sensor Thermostat: ${this.accessory.displayName}` +
                                  ` accessoryAttribute: ${JSON.stringify(
  ...
  public async refreshRoomPriority() {
    if (this.device.thermostat?.roompriority?.deviceType === 'Thermostat') {
      ...
      this.debugLog(
        `Room Sensor Thermostat: ${
          this.accessory.displayName
        } roompriority: ${JSON.stringify(this.roompriority)}`,
      );
    }
  }
  ...
  async pushRoomChanges() {
    this.debugLog(`Room Sensor Thermostat: ${
      this.accessory.displayName
    } Room Priority,
     Current Room: ${JSON.stringify(
    this.roompriority.currentPriority.selectedRooms,
  )}, Changing Room: [${this.sensorAccessory.accessoryId}]`);
    if (
      ...
      if (this.device.thermostat?.roompriority?.deviceType === 'Thermostat') {
        if (this.device.thermostat?.roompriority.priorityType === 'FollowMe') {
          this.platform.log.info(
            `Room Sensor Thermostat: ${this.accessory.displayName} sent request to Honeywell API, Priority Type: ` +
              `${this.device.thermostat?.roompriority.priorityType} Built-in Occupancy Sensor(s) Will be used to set Priority Automatically.`,
          );
        } else if (
          this.device.thermostat?.roompriority.priorityType === 'WholeHouse'
        ) {
          this.platform.log.info(
            `Room Sensor Thermostat: ${this.accessory.displayName} sent request to Honeywell API,` +
              ` Priority Type: ${this.device.thermostat?.roompriority.priorityType}`,
          );
        } else if (
          this.device.thermostat?.roompriority.priorityType === 'PickARoom'
        ) {
          this.platform.log.info(
            `Room Sensor Thermostat: ${this.accessory.displayName} sent request to Honeywell API,` +
  ...
  updateHomeKitCharacteristics() {
    if (this.TemperatureDisplayUnits === undefined) {
      this.debugLog(
        `Room Sensor Thermostat: ${this.accessory.displayName} TemperatureDisplayUnits: ${this.TemperatureDisplayUnits}`,
      );
    } else {
    ...
    if (this.CurrentRelativeHumidity === undefined) {
      this.debugLog(
        `Room Sensor Thermostat: ${this.accessory.displayName} CurrentRelativeHumidity: ${this.CurrentRelativeHumidity}`,
      );
    } else {
      this.service.updateCharacteristic(
    ...
    if (this.TargetTemperature === undefined) {
      this.debugLog(
        `Room Sensor Thermostat: ${this.accessory.displayName} TargetTemperature: ${this.TargetTemperature}`,
      );
    } else {
      this.service.updateCharacteristic(
      ...
      this.debugLog(
        `Room Sensor Thermostat: ${this.accessory.displayName} updateCharacteristic TargetTemperature: ${this.TargetTemperature}`,
      );
    }
    if (this.HeatingThresholdTemperature === undefined) {
      this.debugLog(
        `Room Sensor Thermostat: ${this.accessory.displayName} HeatingThresholdTemperature: ${this.HeatingThresholdTemperature}`,
      );
    } else {
      this.service.updateCharacteristic(
        this.platform.Characteristic.HeatingThresholdTemperature,
    ...
    if (this.TargetHeatingCoolingState === undefined) {
      this.debugLog(
        `Room Sensor Thermostat: ${this.accessory.displayName} TargetHeatingCoolingState: ${this.TargetHeatingCoolingState}`,
      );
    } else {
      this.service.updateCharacteristic(
  ...
  public honeywellAPIError(e: any) {
    if (e.message.includes('400')) {
      this.platform.log.error(
        `Room Sensor Thermostat: ${this.accessory.displayName} failed to ${this.action}, Bad Request`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Room Sensor Thermostat: ${this.accessory.displayName} failed to ${this.action}, Unauthorized Request`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Room Sensor Thermostat: ${this.accessory.displayName} failed to ${this.action}, Forbidden Request`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Room Sensor Thermostat: ${this.accessory.displayName} failed to ${this.action}, Requst Not Found`,
      );
      this.debugLog('Specifies the requested path does not exist.');
    } else if (e.message.includes('406')) {
      this.platform.log.error(
        `Room Sensor Thermostat: ${this.accessory.displayName} failed to ${this.action}, Request Not Acceptable`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Room Sensor Thermostat: ${this.accessory.displayName} failed to ${this.action}, Unsupported Requst Header`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Room Sensor Thermostat: ${this.accessory.displayName} failed to ${this.action}, Unprocessable Entity`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Room Sensor Thermostat: ${this.accessory.displayName} failed to ${this.action}, Too Many Requests`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Room Sensor Thermostat: ${this.accessory.displayName} failed to ${this.action}, Internal Server Error`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Room Sensor Thermostat: ${this.accessory.displayName} failed to ${this.action},`,
      );
    }
    if (this.deviceLogging.includes('debug')) {
      this.platform.log.error(
        `Room Sensor Thermostat: ${this.accessory.displayName} failed to pushChanges, ` +
  ...
  private setTargetHeatingCoolingState(value: homebridge.CharacteristicValue) {
    this.debugLog(
      `Room Sensor Thermostat: ${this.accessory.displayName} Set TargetHeatingCoolingState: ${value}`,
    );
  ...
  private setCoolingThresholdTemperature(
    ...
    this.debugLog(
      `Room Sensor Thermostat: ${this.accessory.displayName} Set CoolingThresholdTemperature: ${value}`,
    );
  ...
  private setTargetTemperature(value: homebridge.CharacteristicValue) {
    this.debugLog(
      `Room Sensor Thermostat: ${this.accessory.displayName} Set TargetTemperature: ${value}`,
    );
  ...
  private setTemperatureDisplayUnits(value: homebridge.CharacteristicValue) {
    this.debugLog(
      `Room Sensor Thermostat: ${this.accessory.displayName} Set TemperatureDisplayUnits: ${value}`,
    );
  ...
  private TargetState() {
    this.debugLog(
      `Room Sensor Thermostat: ${this.accessory.displayName} allowedModes: ${this.device.allowedModes}`,
    );
  ...
  refreshRate(device: settings.device & settings.devicesConfig) {
    if (device.thermostat?.roompriority?.refreshRate) {
      ...
      this.debugLog(
        `Room Sensor Thermostat: ${this.accessory.displayName} Using Device Config refreshRate: ${this.deviceRefreshRate}`,
      );
    } else if (device.refreshRate) {
      this.deviceRefreshRate = this.accessory.context.refreshRate =
        device.refreshRate;
      this.debugLog(
        `Room Sensor Thermostat: ${this.accessory.displayName} Using Thermostat Device Config refreshRate: ${this.deviceRefreshRate}`,
      );
    } else if (this.platform.config.options!.refreshRate) {
      this.deviceRefreshRate = this.accessory.context.refreshRate =
        this.platform.config.options!.refreshRate;
      this.debugLog(
        `Room Sensor Thermostat: ${this.accessory.displayName} Using Platform Config refreshRate: ${this.deviceRefreshRate}`,
      );
    }
  }
  ...
  logs(device: settings.device & settings.devicesConfig) {
    if (this.platform.debugMode) {
      ...
      this.debugLog(
        `Room Sensor Thermostat: ${this.accessory.displayName} Using Debug Mode Logging: ${this.deviceLogging}`,
      );
    } else if (device.thermostat?.roompriority?.logging) {
      this.deviceLogging = this.accessory.context.logging =
        device.thermostat?.roompriority?.logging;
      this.debugLog(
        `Room Sensor Thermostat: ${this.accessory.displayName} Using Device Config Logging: ${this.deviceLogging}`,
      );
    } else if (this.platform.config.options?.logging) {
      this.deviceLogging = this.accessory.context.logging =
        this.platform.config.options?.logging;
      this.debugLog(
        `Room Sensor Thermostat: ${this.accessory.displayName} Using Platform Config Logging: ${this.deviceLogging}`,
      );
    } else {
  ...
  enablingDeviceLogging(): boolean {
    return (
      this.deviceLogging === 'standard' || this.deviceLogging.includes('debug')
    );
  }
}
