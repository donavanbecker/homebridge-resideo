...
export class RoomSensors {
  ...
  constructor(
    ...
    if (device.thermostat?.roomsensor?.hide_temperature) {
      this.debugLog(
        `Room Sensor: ${accessory.displayName} Removing Temperature Sensor Service`,
      );
      ...
      this.debugLog(
        `Room Sensor: ${accessory.displayName} Add Temperature Sensor Service`,
      );
      ...
      this.debugLog(
        `Room Sensor: ${accessory.displayName} Temperature Sensor Service Not Added`,
      );
    }
    ...
    if (device.thermostat?.roomsensor?.hide_occupancy) {
      this.debugLog(
        `Room Sensor: ${accessory.displayName} Removing Occupancy Sensor Service`,
      );
      this.occupancyService = this.accessory.getService(
        this.platform.Service.OccupancySensor,
      );
      accessory.removeService(this.occupancyService!);
    } else if (!this.occupancyService) {
      this.debugLog(
        `Room Sensor: ${accessory.displayName} Add Occupancy Sensor Service`,
      );
      ...
      this.debugLog(
        `Room Sensor: ${accessory.displayName} Occupancy Sensor Service Not Added`,
      );
    }
    ...
    if (device.thermostat?.roomsensor?.hide_humidity) {
      this.debugLog(
        `Room Sensor: ${accessory.displayName} Removing Humidity Sensor Service`,
      );
      ...
      this.debugLog(
        `Room Sensor: ${accessory.displayName} Add Humidity Sensor Service`,
      );
      ...
      this.debugLog(
        `Room Sensor: ${accessory.displayName} Humidity Sensor Service Not Added`,
      );
    }
  ...
  parseStatus() {
    ...
    this.debugLog(
      `Room Sensor: ${this.accessory.displayName} StatusLowBattery: ${this.StatusLowBattery}`,
    );
    ...
    this.debugLog(
      `Room Sensor: ${this.accessory.displayName} CurrentTemperature: ${this.CurrentTemperature}°c`,
    );
    ...
    this.debugLog(
      `Room Sensor: ${this.accessory.displayName} CurrentRelativeHumidity: ${this.CurrentRelativeHumidity}%`,
    );
  }
  ...
  updateHomeKitCharacteristics() {
    if (this.StatusLowBattery === undefined) {
      ...
      this.debugLog(
        `Room Sensor: ${this.accessory.displayName} updateCharacteristic StatusLowBattery: ${this.StatusLowBattery}`,
      );
    }
    if (
      this.device.thermostat?.roomsensor?.hide_temperature ||
    ...
    if (
      this.device.thermostat?.roomsensor?.hide_occupancy ||
      this.OccupancyDetected === undefined
    ) {
      this.debugLog(
        `Room Sensor: ${this.accessory.displayName} OccupancyDetected: ${this.OccupancyDetected}`,
      );
    } else {
      ...
      this.debugLog(
        `Room Sensor: ${this.accessory.displayName} updateCharacteristic OccupancyDetected: ${this.OccupancyDetected}`,
      );
    }
    if (
      this.device.thermostat?.roomsensor?.hide_humidity ||
      ...
      this.debugLog(
        `Room Sensor: ${this.accessory.displayName}` +
  ...
  config(device: device & devicesConfig) {
    ...
    if (Object.entries(config).length !== 0) {
      this.warnLog(
        `Room Sensor: ${this.accessory.displayName} Config: ${JSON.stringify(
          config,
        )}`,
      );
    }
  ...
  refreshRate(device: device & devicesConfig) {
    if (device.thermostat?.roomsensor?.refreshRate) {
      this.deviceRefreshRate = this.accessory.context.refreshRate =
        device.thermostat?.roomsensor?.refreshRate;
      this.debugLog(
        `Room Sensor: ${this.accessory.displayName} Using Device Config refreshRate: ${this.deviceRefreshRate}`,
      );
    } else if (device.refreshRate) {
      this.deviceRefreshRate = this.accessory.context.refreshRate =
        device.refreshRate;
      this.debugLog(
        `Room Sensor: ${this.accessory.displayName} Using Thermostat Device Config refreshRate: ${this.deviceRefreshRate}`,
      );
    } else if (this.platform.config.options!.refreshRate) {
      this.deviceRefreshRate = this.accessory.context.refreshRate =
        this.platform.config.options!.refreshRate;
      this.debugLog(
        `Room Sensor: ${this.accessory.displayName} Using Platform Config refreshRate: ${this.deviceRefreshRate}`,
      );
    }
  }
  ...
  logs(device: device & devicesConfig) {
    if (this.platform.debugMode) {
      ...
      this.deviceLogging = this.accessory.context.logging =
        device.thermostat?.roomsensor?.logging;
      this.debugLog(
        `Room Sensor: ${this.accessory.displayName} Using Device Config Logging: ${this.deviceLogging}`,
      );
