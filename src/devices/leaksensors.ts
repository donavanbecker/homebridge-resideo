...
export class LeakSensor {
  ...
  constructor(
    ...
    if (this.device.leaksensor?.hide_leak) {
      this.debugLog(
        `Leak Sensor: ${accessory.displayName} Removing Leak Sensor Service`,
      );
      this.leakService = this.accessory.getService(
        this.platform.Service.LeakSensor,
      );
      accessory.removeService(this.leakService!);
    } else if (!this.leakService) {
      this.debugLog(
        `Leak Sensor: ${accessory.displayName} Add Leak Sensor Service`,
      );
      (this.leakService =
      ...
      this.leakService.setCharacteristic(
        this.platform.Characteristic.Name,
        `${accessory.displayName} Leak Sensor`,
      );
    } else {
      this.debugLog(
        `Leak Sensor: ${accessory.displayName} Leak Sensor Service Not Added`,
      );
    }
    ...
    if (this.device.leaksensor?.hide_temperature) {
      this.debugLog(
        `Leak Sensor: ${accessory.displayName} Removing Temperature Sensor Service`,
      );
      ...
      this.debugLog(
        `Leak Sensor: ${accessory.displayName} Add Temperature Sensor Service`,
      );
      ...
      this.debugLog(
        `Leak Sensor: ${accessory.displayName} Temperature Sensor Service Not Added`,
      );
    }
    ...
    if (this.device.leaksensor?.hide_humidity) {
      this.debugLog(
        `Leak Sensor: ${accessory.displayName} Removing Humidity Sensor Service`,
      );
      ...
      this.debugLog(
        `Leak Sensor: ${accessory.displayName} Add Humidity Sensor Service`,
      );
      ...
      this.debugLog(
        `Leak Sensor: ${accessory.displayName} Humidity Sensor Service Not Added`,
      );
    }
  ...
  parseStatus() {
    ...
    this.debugLog(
      `Leak Sensor: ${this.accessory.displayName} LeakDetected: ${this.LeakDetected}`,
    );
    ...
    if (!this.device.leaksensor?.hide_humidity) {
      ...
      this.debugLog(
        `Leak Sensor: ${this.accessory.displayName} CurrentRelativeHumidity: ${this.CurrentRelativeHumidity}%`,
      );
    }
    ...
    this.debugLog(
      `Leak Sensor: ${this.accessory.displayName} BatteryLevel: ${this.BatteryLevel},` +
  ...
  async refreshStatus() {
    try {
      ...
      this.device = device;
      this.debugLog(
        `Leak Sensor: ${this.accessory.displayName} device: ${JSON.stringify(
          this.device,
        )}`,
      );
      this.parseStatus();
  ...
  updateHomeKitCharacteristics() {
    if (this.BatteryLevel === undefined) {
      ...
      this.debugLog(
        `Leak Sensor: ${this.accessory.displayName} updateCharacteristic BatteryLevel: ${this.BatteryLevel}`,
      );
    }
    if (this.StatusLowBattery === undefined) {
      this.debugLog(
        `Leak Sensor: ${this.accessory.displayName} StatusLowBattery: ${this.StatusLowBattery}`,
      );
    } else {
      this.service.updateCharacteristic(
      ...
      this.debugLog(
        `Leak Sensor: ${this.accessory.displayName} updateCharacteristic StatusLowBattery: ${this.StatusLowBattery}`,
      );
    }
    if (!this.device.leaksensor?.hide_leak) {
      if (this.LeakDetected === undefined) {
        ...
        this.debugLog(
          `Leak Sensor: ${this.accessory.displayName} updateCharacteristic LeakDetected: ${this.LeakDetected}`,
        );
      }
      if (this.StatusActive === undefined) {
        this.debugLog(
          `Leak Sensor: ${this.accessory.displayName} StatusActive: ${this.StatusActive}`,
        );
      } else {
        this.leakService?.updateCharacteristic(
        ...
        this.debugLog(
          `Leak Sensor: ${this.accessory.displayName} updateCharacteristic StatusActive: ${this.StatusActive}`,
        );
      }
    }
    if (
      this.device.leaksensor?.hide_temperature &&
    ...
    if (
      this.device.leaksensor?.hide_humidity &&
      ...
      this.debugLog(
        `Leak Sensor: ${this.accessory.displayName}` +
  ...
  public honeywellAPIError(e: any) {
    if (e.message.includes("400")) {
      this.platform.log.error(
        `Leak Sensor: ${this.accessory.displayName} failed to ${this.action}, Bad Request`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Leak Sensor: ${this.accessory.displayName} failed to ${this.action}, Unauthorized Request`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Leak Sensor: ${this.accessory.displayName} failed to ${this.action}, Forbidden Request`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Leak Sensor: ${this.accessory.displayName} failed to ${this.action}, Requst Not Found`,
      );
      this.debugLog("Specifies the requested path does not exist.");
    } else if (e.message.includes("406")) {
      this.platform.log.error(
        `Leak Sensor: ${this.accessory.displayName} failed to ${this.action}, Request Not Acceptable`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Leak Sensor: ${this.accessory.displayName} failed to ${this.action}, Unsupported Requst Header`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Leak Sensor: ${this.accessory.displayName} failed to ${this.action}, Unprocessable Entity`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Leak Sensor: ${this.accessory.displayName} failed to ${this.action}, Too Many Requests`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Leak Sensor: ${this.accessory.displayName} failed to ${this.action}, Internal Server Error`,
      );
      this.debugLog(
      ...
      this.platform.log.error(
        `Leak Sensor: ${this.accessory.displayName} failed to ${this.action},`,
      );
    }
    ...
    if (this.deviceLogging.includes("debug") || this.platform.debugMode) {
      this.platform.log.error(
        `Leak Sensor: ${this.accessory.displayName} Error: ${JSON.stringify(e)}`,
      );
    }
  }
  ...
  config(device: device & devicesConfig) {
    ...
    if (Object.entries(config).length !== 0) {
      this.warnLog(
        `Leak Sensor: ${this.accessory.displayName} Config: ${JSON.stringify(
          config,
        )}`,
      );
    }
  ...
  refreshRate(device: device & devicesConfig) {
    if (device.refreshRate) {
      this.deviceRefreshRate = this.accessory.context.refreshRate =
        device.refreshRate;
      this.debugLog(
        `Leak Sensor: ${this.accessory.displayName} Using Device Config refreshRate: ${this.deviceRefreshRate}`,
      );
    } else if (this.platform.config.options!.refreshRate) {
      this.deviceRefreshRate = this.accessory.context.refreshRate =
        this.platform.config.options!.refreshRate;
      this.debugLog(
        `Leak Sensor: ${this.accessory.displayName} Using Platform Config refreshRate: ${this.deviceRefreshRate}`,
      );
    }
  }
  ...
  logs(device: device & devicesConfig) {
    if (this.platform.debugMode) {
      ...
      this.debugLog(
        `Leak Sensor: ${this.accessory.displayName} Using Debug Mode Logging: ${this.deviceLogging}`,
      );
    } else if (device.logging) {
      this.deviceLogging = this.accessory.context.logging = device.logging;
      this.debugLog(
        `Leak Sensor: ${this.accessory.displayName} Using Device Config Logging: ${this.deviceLogging}`,
      );
    } else if (this.platform.config.options?.logging) {
      this.deviceLogging = this.accessory.context.logging =
        this.platform.config.options?.logging;
      this.debugLog(
        `Leak Sensor: ${this.accessory.displayName} Using Platform Config Logging: ${this.deviceLogging}`,
      );
    } else {
